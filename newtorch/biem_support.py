...
def allExactStokesSLTarget_compare1(
    ...
    stokesSLPtar = torch.zeros(
        (2 * Ntar, ntar), dtype=torch.float64, device=vesicleX.device
    )
...
def wrapper_allExactStokesSLTarget_compare2(
    vesicleX, vesicle_sa, fup, tarX, info_stokes
):
    nv = tarX.shape[1]
        ...
        far_field_1 = torch.concat(
            (
                ...
                fn(
                    ...
                    tarX[:, nv // 2 : 3 * nv // 4],
                    info_stokes[0][
                        (nv // 2 <= info_stokes[2]) & (info_stokes[2] < 3 * nv // 4)
                    ],
                    info_stokes[1][
                        (nv // 2 <= info_stokes[2]) & (info_stokes[2] < 3 * nv // 4)
                    ],
...
def exactStokesSL_(vesicle, f, Xtar=None, K1=None):
    ...
    ncol = Xtar.shape[1]
    stokesSLPtar = torch.zeros(
        (2 * Ntar, ncol), dtype=torch.float64, device=Xtar.device
    )
...
def exactStokesSL_onlyself_old(vesicleUp, fup, X, vself, nlayers=3, upsample=-1):
    ...
    tracers = torch.permute(
        torch.vstack(
            [
                torch.repeat_interleave(Xup[:Nup, :, None], nlayers - 1, dim=-1) + dx,
                torch.repeat_interleave(Xup[Nup:, :, None], nlayers - 1, dim=-1) + dy,
            ]
        ),
        (0, 2, 1),
    )  # (2*N, nlayers-1, nv)
    ...
    vel_self_and_layer = torch.concat(
        (vself[:, None], vel_layers), dim=1
    )  # (2*N, nlayers, nv)
    tracers = torch.concat((Xup[:, None], tracers), dim=1)

    return (
        vel_self_and_layer[:Nup],
        vel_self_and_layer[Nup:],
        tracers[:Nup],
        tracers[Nup:],
    )
...
@torch.jit.script
def exactStokesSL_onlyself(
    vesicleUpX, vesicleUp_sa, fup, Nup: int, Xup, vself, tracers
):
    """
    Computes the single-layer potential due to `f` around all vesicles except itself.
    Also can pass a set of target points `Xtar` and a collection of vesicles `K1`
    ...
    xsou = vesicleUpX[:vesicleUp_N, :]
    ysou = vesicleUpX[vesicleUp_N:, :]
    xtar = tracers[:Nup, :, :]
    ytar = tracers[Nup:, :, :]
    denx = den[:vesicleUp_N, ...]
    deny = den[vesicleUp_N:, ...]

    diffx = xtar[None, :] - xsou[:, None, None]
    diffy = ytar[None, :] - ysou[:, None, None]
    ...
    vel_self_and_layer = torch.concat((vself[:, None], vel_layers), dim=1)
    tracers = torch.concat((Xup[:, None], tracers), dim=1)
...
def naiveNearZoneInfo(vesicleX, vesicleUpX, max_layer_dist=None):
    ...
    max_layer_dist = 1.0 / N if max_layer_dist is None else max_layer_dist
    ...
    all_points_up = torch.concat(
        (vesicleUpX[:Nup, :].T.reshape(-1, 1), vesicleUpX[Nup:, :].T.reshape(-1, 1)),
        dim=1,
    )
    ...
    closest_dist, idx_cloest = torch.min(
        distances.reshape(nv, N, nv, Nup), dim=1
    )  # (nv, nv, Nup)
...
def dist_wrapper_allExactStokesSLTarget_compare2(
    vesicleX, vesicle_sa, fup, tarX, info_stokes, base_offset=0
):
    nv = tarX.shape[1]
    fn = allExactStokesSLTarget_compare2
    ...
    far_field_1 = fn(
        vesicleX, vesicle_sa, fup, tarX, ids0, ids1, ids2_local, offset=base_offset
    )

    return far_field_1


def scalableNearZoneInfo(
    vesicleX,
    Nup,
    local_start: int = 0,
    local_end: int = None,
    max_layer_dist=None,
    center_pad: float = 2.5,
):
    if local_end is None:
        local_end = vesicleX.shape[1]

    device = vesicleX.device
    dtype = vesicleX.dtype
    N = vesicleX.shape[0] // 2
    nv = vesicleX.shape[1]
    if local_end <= local_start:
        empty = torch.empty(0, dtype=torch.long, device=device)
        return (None, None, (empty, empty), None, (empty, empty, empty))

    max_layer_dist = (1.0 / N) if max_layer_dist is None else max_layer_dist

    x = vesicleX[:N]
    y = vesicleX[N:]
    centers_x = x.mean(dim=0)
    centers_y = y.mean(dim=0)
    radii = torch.sqrt((x - centers_x) ** 2 + (y - centers_y) ** 2).max(dim=0).values

    local_ids = torch.arange(local_start, local_end, device=device)
    local_centers_x = centers_x[local_start:local_end]
    local_centers_y = centers_y[local_start:local_end]
    local_radii = radii[local_start:local_end]

    pair_dx = local_centers_x[:, None] - centers_x[None, :]
    pair_dy = local_centers_y[:, None] - centers_y[None, :]
    pair_dist = torch.sqrt(pair_dx * pair_dx + pair_dy * pair_dy)
    pair_thresh = center_pad * max_layer_dist + local_radii[:, None] + radii[None, :]
    pair_mask = pair_dist <= pair_thresh
    pair_mask[torch.arange(local_end - local_start, device=device), local_ids] = False

    cand_local_tars, cand_sources = torch.where(pair_mask)
    if cand_sources.numel() == 0:
        empty = torch.empty(0, dtype=torch.long, device=device)
        return (None, None, (empty, empty), None, (empty, empty, empty))
    ...
    for lt, src in zip(cand_local_tars.tolist(), cand_sources.tolist()):
        gtar = local_start + lt
        xt = x[:, gtar]
        yt = y[:, gtar]
        xs = x[:, src]
        ...
        diffx = xt[:, None] - xs[None, :]
        diffy = yt[:, None] - ys[None, :]
        dis2 = diffx * diffx + diffy * diffy

        near_t = torch.any(dis2 < (max_layer_dist * max_layer_dist), dim=1)
        J = torch.where(near_t)[0]
        if J.numel() == 0:
            continue

        id1_list.append(gtar * N + J)
        id2_list.append(torch.full((J.numel(),), src, dtype=torch.long, device=device))

        pair_near = torch.any(dis2 < ((1.0 / Nup) ** 2), dim=1)
        J2 = torch.where(pair_near)[0]
        if J2.numel() > 0:
            ids0_list.append(
                torch.full(
                    (J2.numel(),),
                    src - (1 if gtar <= src else 0),
                    dtype=torch.long,
                    device=device,
                )
            )
            ids1_list.append(J2)
            ids2_list.append(
                torch.full((J2.numel(),), gtar, dtype=torch.long, device=device)
            )

    if id1_list:
        id1 = torch.cat(id1_list)
        id2 = torch.cat(id2_list)
    else:
        id1 = torch.empty(0, dtype=torch.long, device=device)
        id2 = torch.empty(0, dtype=torch.long, device=device)

    if ids0_list:
        ids0 = torch.cat(ids0_list)
        ids1 = torch.cat(ids1_list)
        ids2 = torch.cat(ids2_list)
    else:
        ids0 = torch.empty(0, dtype=torch.long, device=device)
        ids1 = torch.empty(0, dtype=torch.long, device=device)
        ids2 = torch.empty(0, dtype=torch.long, device=device)

    return (None, None, (id1, id2), None, (ids0, ids1, ids2))
