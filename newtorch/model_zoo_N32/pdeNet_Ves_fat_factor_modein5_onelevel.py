...
# input: coords 1ch, mode 1ch, two channels (2,256)
# starter: in 2ch, out 11ch= 10ch + mode 1ch, mode is duplicated
# evo: in 11ch, out 11ch = 10ch + mode 1ch
# end: in 11ch, out 3ch = 2ch + mode 1ch
...
class Starter_Block(nn.Module):
    ...
    def forward(self, input):
        ...
        # out = self.mix1(out)
        out = self.mix(out)
        out = torch.cat((out, input[:,1].reshape(-1,1,N)), dim=1)
...
class Evo_Block(nn.Module):
    def __init__(self, factor, ch):
        ...
        self.conv1_1 = nn.Conv1d(ch+1, conv1_out_ch, kernel_size=7, stride=2, padding=3, padding_mode='circular')
        self.conv1_2 = nn.Conv1d(ch+1, conv1_out_ch, kernel_size=7, stride=2, padding=6, dilation=2, padding_mode='circular')
        self.bn1 = nn.BatchNorm1d(num_features=2*conv1_out_ch)
        self.relu = nn.ReLU(inplace=True)
        # conv2_out_ch = math.floor(10*factor)
...
class pdeNet_Ves_fat_factor_modein5_onelevel(nn.Module):
    def __init__(self, num_blocks, factor, ch):
        super(pdeNet_Ves_fat_factor_modein5_onelevel, self).__init__()
        self.factor = factor
        self.ch = ch
...
# model = pdeNet_Ves_fat_factor_modein5_onelevel(num_blocks=11, factor=2.0, ch=26)
# summary(model, (2, 64))

# model.load_state_dict(torch.load("ves_mixedmode_12blks_252984.pth", map_location=torch.device("cpu")))
# print("hello")
