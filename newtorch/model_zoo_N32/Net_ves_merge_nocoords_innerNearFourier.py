...
class End_Block(nn.Module):
    def __init__(self, factor, ch, rep):
        ...
        self.mix = nn.Conv1d(deconv_ch, 8*rep, kernel_size=3, stride=1, padding=1, groups=rep)
...
class Net_ves_merge_nocoords_innerNearFourier(nn.Module):
    def __init__(self, num_blocks, factor, ch, rep):
        super(Net_ves_merge_nocoords_innerNearFourier, self).__init__()
        self.factor = factor
        self.ch = ch
        self.rep = rep
