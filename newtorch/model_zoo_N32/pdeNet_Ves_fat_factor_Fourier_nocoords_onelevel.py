...
class End_Block(nn.Module):
    def __init__(self, factor, ch):
        ...
        self.mix = nn.Conv1d(deconv_ch, 8, kernel_size=3, stride=1, padding=1)
        # self.W = nn.Conv1d(1, 1, kernel_size=3, stride=1, padding=1)
...
class pdeNet_Ves_fat_factor_Fourier_nocoords_onelevel(nn.Module):
    def __init__(self, num_blocks, factor, ch):
        super(pdeNet_Ves_fat_factor_Fourier_nocoords_onelevel, self).__init__()
        self.factor = factor
        self.ch = ch
...
model = pdeNet_Ves_fat_factor_Fourier_nocoords_onelevel(num_blocks=12, factor=3.2, ch=30)
summary(model, (2, 32))
