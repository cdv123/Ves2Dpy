...
class Starter_Block(nn.Module):
    def __init__(self, factor, ch, rep):
        ...
        self.conv1_1 = nn.Conv1d(2*rep, conv1_out_ch*rep, kernel_size=7, stride=1, padding=3, padding_mode='circular', groups=rep)
        self.conv1_2 = nn.Conv1d(2*rep, conv1_out_ch*rep, kernel_size=7, stride=1, padding=6, dilation=2, padding_mode='circular', groups=rep)
        self.bn1 = nn.BatchNorm1d(num_features=2*conv1_out_ch*rep)
        self.relu = nn.ReLU(inplace=True)
        conv2_out_ch = math.floor(10*factor)
        self.conv2_1 = nn.Conv1d(2*conv1_out_ch*rep, conv2_out_ch*rep, kernel_size=7, stride=1, padding=3, padding_mode='circular', groups=rep)
        self.conv2_2 = nn.Conv1d(2*conv1_out_ch*rep, conv2_out_ch*rep, kernel_size=7, stride=1, padding=6, dilation=2, padding_mode='circular', groups=rep)
        deconv_ch = 2*conv2_out_ch*rep
        self.bn2 = nn.BatchNorm1d(num_features=deconv_ch)
        # self.deconv1 = nn.ConvTranspose1d(deconv_ch, deconv_ch, kernel_size=10, stride=1, padding=4, groups=rep)
        # self.bn3 = nn.BatchNorm1d(num_features=deconv_ch)
        # self.deconv2 = nn.ConvTranspose1d(deconv_ch, deconv_ch, kernel_size=10, stride=1, padding=4, groups=rep)
        # self.bn4 = nn.BatchNorm1d(num_features=deconv_ch)

        self.conv3_1 = nn.Conv1d(2*conv2_out_ch*rep, conv2_out_ch*rep, kernel_size=7, stride=1, padding=3, padding_mode='circular', groups=rep)
        self.conv3_2 = nn.Conv1d(2*conv2_out_ch*rep, conv2_out_ch*rep, kernel_size=7, stride=1, padding=6, dilation=2, padding_mode='circular', groups=rep)
        self.bn3 = nn.BatchNorm1d(num_features=deconv_ch)

        self.conv4_1 = nn.Conv1d(2*conv2_out_ch*rep, conv2_out_ch*rep, kernel_size=7, stride=1, padding=3, padding_mode='circular', groups=rep)
        self.conv4_2 = nn.Conv1d(2*conv2_out_ch*rep, conv2_out_ch*rep, kernel_size=7, stride=1, padding=6, dilation=2, padding_mode='circular', groups=rep)
        self.bn4 = nn.BatchNorm1d(num_features=deconv_ch)
    ...
    def forward(self, input):
        ...
        ch = out_3_1.shape[1]//self.rep
        # features = [torch.cat((out_2_1[:, (ch)*i:(ch)*(i+1)],
        #                        out_2_2[:, (ch)*i:(ch)*(i+1)],),dim=1)  for i in range(self.rep)]
        # out = torch.cat(tuple(features), dim=1)
        out = torch.cat((out_3_1.reshape(bs, self.rep, ch, -1), out_3_2.reshape(bs, self.rep, ch, -1)), dim=2)
        out = out.reshape(bs, 2*ch*self.rep, -1)
        out = self.bn3(out)

        out_4_1 = self.relu(self.conv4_1(out))
        out_4_2 = self.relu(self.conv4_2(out))
        ch = out_4_1.shape[1]//self.rep
        # features = [torch.cat((out_2_1[:, (ch)*i:(ch)*(i+1)],
        #                        out_2_2[:, (ch)*i:(ch)*(i+1)],),dim=1)  for i in range(self.rep)]
        # out = torch.cat(tuple(features), dim=1)
        out = torch.cat((out_4_1.reshape(bs, self.rep, ch, -1), out_4_2.reshape(bs, self.rep, ch, -1)), dim=2)
        out = out.reshape(bs, 2*ch*self.rep, -1)
        out = self.bn4(out)
...
class Evo_Block(nn.Module):
    def __init__(self, factor, ch, rep):
        ...
        self.conv1_1 = nn.Conv1d(ch*rep, conv1_out_ch*rep, kernel_size=7, stride=1, padding=3, padding_mode='circular', groups=rep)
        self.conv1_2 = nn.Conv1d(ch*rep, conv1_out_ch*rep, kernel_size=7, stride=1, padding=6, dilation=2, padding_mode='circular', groups=rep)
        self.bn1 = nn.BatchNorm1d(num_features=2*conv1_out_ch*rep)
        self.relu = nn.ReLU(inplace=True)
        conv2_out_ch = math.floor(10*factor)
        self.conv2_1 = nn.Conv1d(2*conv1_out_ch*rep, conv2_out_ch*rep, 
                                 kernel_size=7, stride=1, padding=3, padding_mode='circular', groups=rep)
        self.conv2_2 = nn.Conv1d(2*conv1_out_ch*rep, conv2_out_ch*rep, 
                                 kernel_size=7, stride=1, padding=6, dilation=2, padding_mode='circular', groups=rep)
        deconv_ch = 2*conv2_out_ch*rep
        self.bn2 = nn.BatchNorm1d(num_features=deconv_ch)
        # self.deconv1 = nn.ConvTranspose1d(deconv_ch, deconv_ch, kernel_size=10, stride=1, padding=4, groups=rep)
        # self.bn3 = nn.BatchNorm1d(num_features=deconv_ch)
