...
class Evo_Block(nn.Module):
    def __init__(self, factor, ch):
        ...
        self.conv1_1 = nn.Conv1d(ch+2, conv1_out_ch, kernel_size=7, stride=1, padding=3, padding_mode='circular')
        self.conv1_2 = nn.Conv1d(ch+2, conv1_out_ch, kernel_size=7, stride=1, padding=6, dilation=2, padding_mode='circular')
        self.bn1 = nn.BatchNorm1d(num_features=2*conv1_out_ch)
        self.relu = nn.ReLU(inplace=True)
        conv2_out_ch = math.floor(10*factor)
...
class End_Block(nn.Module):
    def __init__(self, factor, ch):
        ...
        self.mix = nn.Conv1d(deconv_ch, 1, kernel_size=3, stride=1, padding=1)
        # self.W = nn.Conv1d(1, 1, kernel_size=3, stride=1, padding=1)
...
class pdeNet_Ves_fat_factor_modein6_zerolevel(nn.Module):
    def __init__(self, num_blocks, factor, ch):
        super(pdeNet_Ves_fat_factor_modein6_zerolevel, self).__init__()
        self.factor = factor
        self.ch = ch
...
# model = pdeNet_Ves_fat_factor_modein6_zerolevel(num_blocks=12, factor=1.3, ch=16)
# summary(model, (2, 32))
