...
class Evo_Block(nn.Module):
    def __init__(self, factor, ch):
        ...
        self.conv1_1 = nn.Conv1d(ch+2, conv1_out_ch, kernel_size=7, stride=2, padding=3, padding_mode='circular')
        self.conv1_2 = nn.Conv1d(ch+2, conv1_out_ch, kernel_size=7, stride=2, padding=6, dilation=2, padding_mode='circular')
        self.bn1 = nn.BatchNorm1d(num_features=2*conv1_out_ch)
        self.relu = nn.ReLU(inplace=True)
        conv2_out_ch = math.floor(10*factor)
...
class Net_ves_selften(nn.Module):
    def __init__(self, num_blocks, factor, ch):
        super(Net_ves_selften, self).__init__()
        self.factor = factor
        self.ch = ch
