...
def scalableNearZoneInfo(vesicleX, Nup, local_start: int = 0, local_end: int = None, max_layer_dist=None, center_pad: float = 2.5):
    """
    Build near-zone information for only the local target vesicles.

    This avoids the global O((N*nv)^2) cdist/mask construction in naiveNearZoneInfo.
    It first culls vesicle pairs using center/radius bounds, then computes pointwise
    distances only for surviving source-target vesicle pairs.

    Returns a tuple compatible with the distributed near-singular path:
      (None, None, (id1_global, id2_global), None, (ids0, ids1, ids2_global))

    where
      id1_global: global target point ids in [0, N * nv)
      id2_global: source vesicle ids in [0, nv)
      ids0, ids1, ids2_global: exact-Stokes sparse masking triplet matching the
        shape convention expected by dist_wrapper_allExactStokesSLTarget_compare2.
    """
    ...
    for lt, src in zip(cand_local_tars.tolist(), cand_sources.tolist()):
        ...
        if J2.numel() > 0:
            ids0_list.append(torch.full((J2.numel(),), src - (1 if gtar <= src else 0), dtype=torch.long, device=device))
            ids1_list.append(J2)
            ids2_list.append(torch.full((J2.numel(),), gtar, dtype=torch.long, device=device))
